...
def render_pet_card(row: pd.Series, highlight: bool = False):
    ...
    bg = "#FFFFFF" if highlight else "#FFFFFF"
    border_left = "5px solid #ff6b9d" if highlight else "5px solid #ff6b9d"
...
<style>
    ...
    /* Pet card styling - WHITE background */
    .pet-card {
        background: white;
        border-radius: 15px;
        padding: 1.5rem;
    ...
    /* Chat input fixed at bottom - enhanced glass effect */
    ...
    .stChatInputContainer {
        ...
        backdrop-filter: blur(10px);
        background-color: rgba(255, 247, 245, 0.9) !important;
        z-index: 9999 !important;
    }
    
    /* Ensure the input box itself stays visible and styled */
...
def main():
    ...
    # Get the latest message if it hasn't been processed yet
    ...
    # (already displayed above in the loop, so no need to display again)
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            if not env_ok:
                ...
                # Reset clicked buttons and processing flag
                st.session_state.clicked_buttons.clear()
                st.session_state.processing = False
                st.divider()
